...
def test_rectification_pro_can_select_major_accident_v2_draft_card_explicitly(monkeypatch, tmp_path) -> None:
    client = _build_client(monkeypatch, tmp_path)
    payload = _payload(1)
    payload["events"][0]["event_type"] = "major_accident"
    payload["events"][0]["title"] = "Major accident"
    payload["settings"]["formula_card_id"] = "RECT_MAJOR_ACCIDENT_002_DRAFT"
    ...
    body = response.json()
    result = body["formula_test_mode_results"][0]
    assert result["card_id"] == "RECT_MAJOR_ACCIDENT_002_DRAFT"
    assert result["status"] == "draft"
    ...
    assert body["formula_refinement_results"]["card_id"] == "RECT_MAJOR_ACCIDENT_002_DRAFT"


def test_rectification_pro_can_select_local_relocation_v2_draft_card_explicitly(monkeypatch, tmp_path) -> None:
    client = _build_client(monkeypatch, tmp_path)
    payload = _payload(1)
    payload["events"][0]["event_type"] = "local_relocation"
    payload["events"][0]["title"] = "Local relocation"
    payload["settings"]["formula_card_id"] = "RECT_LOCAL_RELOCATION_002_DRAFT"
    ...
    body = response.json()
    result = body["formula_test_mode_results"][0]
    assert result["card_id"] == "RECT_LOCAL_RELOCATION_002_DRAFT"
    assert result["status"] == "draft"
    assert result["formulas_count"] == 88
    assert result["priority_counts"] == {
        "golden": 32,
        "supporting": 26,
        "context": 30,
    ...
    assert body["formula_refinement_results"]["card_id"] == "RECT_LOCAL_RELOCATION_002_DRAFT"


def test_rectification_pro_can_select_long_distance_relocation_v2_draft_card_explicitly(monkeypatch, tmp_path) -> None:
    client = _build_client(monkeypatch, tmp_path)
    payload = _payload(1)
    payload["events"][0]["event_type"] = "long_distance_relocation"
    payload["events"][0]["title"] = "Long-distance relocation"
    payload["settings"]["formula_card_id"] = "RECT_LONG_DISTANCE_RELOCATION_002_DRAFT"
    ...
    body = response.json()
    result = body["formula_test_mode_results"][0]
    assert result["card_id"] == "RECT_LONG_DISTANCE_RELOCATION_002_DRAFT"
    assert result["status"] == "draft"
    assert result["formulas_count"] == 98
    assert result["priority_counts"] == {
        "golden": 36,
        "supporting": 32,
        "context": 30,
    ...
    assert body["formula_refinement_results"]["card_id"] == "RECT_LONG_DISTANCE_RELOCATION_002_DRAFT"


def test_rectification_pro_can_select_surgery_v2_draft_card_explicitly(monkeypatch, tmp_path) -> None:
    client = _build_client(monkeypatch, tmp_path)
    payload = _payload(1)
    payload["events"][0]["event_type"] = "surgery"
    payload["events"][0]["title"] = "Surgical operation"
    payload["settings"]["formula_card_id"] = "RECT_SURGERY_002_DRAFT"
    ...
    body = response.json()
    result = body["formula_test_mode_results"][0]
    assert result["card_id"] == "RECT_SURGERY_002_DRAFT"
    assert result["status"] == "draft"
