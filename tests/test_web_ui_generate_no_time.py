...
def _fake_chart_proxy(**kwargs) -> _FakeResponse:
    path = kwargs["path"]
    payload = kwargs["payload"]
    assert path == "/api/v1/chart"
...
def test_generate_no_time_accepts_birth_date_only_and_sets_facts(monkeypatch) -> None:
    monkeypatch.setattr(web_main, "_post_to_api_with_fallback", _fake_chart_proxy)
    monkeypatch.setattr(
        web_main,
        "_render_no_time_forecast_via_openai",
        lambda prompt_text, forecast_context: {"text": "NO_TIME_OK", "llm_debug": {"provider": "test"}},
        raising=False,
    )
    monkeypatch.setattr(
        web_main,
...
def test_generate_no_time_excludes_natal_moon_but_keeps_minor_supported(monkeypatch) -> None:
    monkeypatch.setattr(web_main, "_post_to_api_with_fallback", _fake_chart_proxy)
    monkeypatch.setattr(
        web_main,
        "_render_no_time_forecast_via_openai",
...
def test_generate_full_datetime_mode_stays_on_old_path(monkeypatch) -> None:
    monkeypatch.setattr(web_main, "_post_to_api_with_fallback", lambda **kwargs: _FakeResponse(_natal_chart_payload("1990-01-15T09:00:00Z")))
    captured: dict[str, Any] = {"full_called": False}

    def _fake_full_render(prompt_text: str, chart: dict[str, Any], core_identity: dict[str, Any]) -> str:
